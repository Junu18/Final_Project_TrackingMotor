...
module top (
    ...
    logic [ 9:0]      x_coor_auto;
    logic [ 9:0]      y_coor_auto;
    logic             red_detect_auto;
    ...
    // keyboard
    assign btn_start = w_keyboard_data[6];
    assign fpga_reset = w_keyboard_data[7];
    ...
    assign reset = sys_reset || ~locked || fpga_reset;
    ...
    mux_nx1 U_mux_nx1 (
        ...
        // miso auto
        .x_coor_auto    (x_coor_auto),
        .y_coor_auto    (y_coor_auto),
        .red_detect_auto(red_detect_auto),
        .shoot_auto     (shoot_auto),

        // manual
        .target_off_manual(1'b0),
    ...
    slave_top U_SPI (
        ...
        .mortor_xdata(mortor_xdata),  // 8bit 180
        .mortor_ydata(mortor_ydata),  // 7bit 90
        .mosi_etc(mosi_etc),
        .mosi_valid(mosi_valid)
    );

    enemy_predict_2s enemy_predict_2s (
        .clk           (sys_clk),
        .reset         (reset),
        .x_meas_now    (aim_x),
        .y_meas_now    (aim_y),
        .red_detect_in (aim_detect_auto),
        .x_predict_out (x_coor_auto),
        .y_predict_out (y_coor_auto),
        .vir_red_detect(red_detect_auto),
        .vir_signal    (w_vir_signal)
    );
